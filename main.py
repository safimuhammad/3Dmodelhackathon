...
async def get_model(
    prompt: str, filename: str, seed: int = 1, guidance: int = 16, steps: int = 64
):
    try:
        ...
        if result:
            ...
            if os.path.exists(result):
                ...
                raise HTTPException(
                    status_code=500,
                    detail="Prediction successful, but no valid result file received.",
                )
        else:
            raise HTTPException(
                status_code=500, detail="Prediction failed or no result received."
            )
    except requests.exceptions.RequestException as req_err:
        raise HTTPException(
            status_code=500, detail=f"An error occurred during the request: {req_err}"
        )
...
async def add_model(
    ...
    if get_model:
        output_path = combine_model(
            f"models/{model1}.glb",
            f"models/{filename}.glb",
            f"{model1}_{filename}",
            color1,
            color2,
        )
        ...
        return FileResponse(
            output_path,
            media_type="application/octet-stream",
            filename=f"{model1}_{filename}.glb",
        )
...
async def list_models():
    models_directory = (
        "models"  # Change this to the actual path of your models directory
    )

    glb_files = [
        file[:-4] for file in os.listdir(models_directory) if file.endswith(".glb")
    ]
...
def change_color(model1, model2, color1, color2):
    ...
    return FileResponse(
        output_path,
        media_type="application/octet-stream",
        filename=f"{model1}_{model2}.glb",
    )
